...
const ExerciseDetailsForm = ({ onSubmit }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label htmlFor="exerciseName" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
          Exercise Name
        </label>
        <input
          type="text"
  ...
  return (
    <div className="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
      <div className="px-4 py-5 sm:px-6">
        <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-white">{result.exercise_name}</h3>
        ...
        <dl className="sm:divide-y sm:divide-gray-200 dark:divide-gray-700">
          {result.description && (
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd className="mt-1 text-sm text-gray-900 dark:text-white sm:mt-0 sm:col-span-2">{result.description}</dd>
            </div>
          )}
          {result.primary_muscles && result.primary_muscles.length > 0 && (
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Primary Muscles</dt>
          ...
          {result.secondary_muscles && result.secondary_muscles.length > 0 && (
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Secondary Muscles</dt>
              <dd className="mt-1 text-sm text-gray-900 dark:text-white sm:mt-0 sm:col-span-2">
          ...
          {result.equipment_needed && result.equipment_needed.length > 0 && (
            <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Equipment Needed</dt>
              <dd className="mt-1 text-sm text-gray-900 dark:text-white sm:mt-0 sm:col-span-2">
        ...
        <div className="max-w-3xl mx-auto">
          <div className="rounded-lg overflow-hidden">
            <div className="px-4 py-5 sm:p-6">
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-6">Exercise Details</h1>
              {isLoading ? (
                <p className="text-center text-gray-600 dark:text-gray-300">Loading exercise details...</p>
              ) : error ? (
